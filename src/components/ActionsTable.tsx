...
function ActionRow({ 
  ...
  const runsPerWeek = timeRangeWeeks > 0 ? (group.runs.length / timeRangeWeeks).toFixed(1) : group.runs.length;
  ...
  const avgPassRuntime = successRuns.length > 0 ? successRuns.reduce((acc, r) => acc + (r.duration || 0), 0) / successRuns.length : null;
  const avgFailRuntime = failRuns.length > 0 ? failRuns.reduce((acc, r) => acc + (r.duration || 0), 0) / failRuns.length : null;
  ...
  return (
    <div className={`relative mb-1 p-2 bg-white dark:bg-gray-800 rounded shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col gap-2 transition-all ${isExpanded ? 'ring-2 ring-blue-100 dark:ring-blue-900' : ''}`}>
      <button 
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
        <div className="flex items-center sm:w-1/3 min-w-0 gap-2 ml-2">
          ...
          <div className="flex justify-between items-center flex-1 min-w-0">
            <h4 className="text-sm font-semibold truncate mr-2 cursor-help" title={tooltipTitle}>{group.name}</h4>
          </div>
        </div>
        ...
        <div className="mt-2 pt-4 border-t border-gray-100 dark:border-gray-700 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="md:col-span-1 space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{avgPassRuntime !== null ? `${avgPassRuntime.toFixed(1)}s` : 'n/a'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">Avg Fail Runtime:</span>
              <span className="font-medium">{avgFailRuntime !== null ? `${avgFailRuntime.toFixed(1)}s` : 'n/a'}</span>
            </div>
            <div className="flex justify-between">
    ...
    <div className="p-4">
      <div className="flex justify-between items-center mb-4">
        ...
        <label className="flex items-center space-x-2 text-sm cursor-pointer select-none">
          ...
          />
          <span className="text-gray-700 dark:text-gray-300">Exclude short runs (&lt; 5% of p90)</span>
        </label>
      </div>
